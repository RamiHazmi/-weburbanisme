...
$id_user =  $_SESSION['user_id']; // 🔁 Later: use $_SESSION['id_user']
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_rental'])) {
        ...
        // Debug log
        error_log("Attempting to delete rental ID: " . $rental_id);
        ...
        if ($rental) {
            error_log("Found rental: " . print_r($rental, true));
            
            if ($rental['end_time'] === null) {
                error_log("Rental is ongoing, attempting to increment bikes");
                // 🚲 Location encore en cours → réincrémenter la station de départ
                $start_station_id = $rental['start_station'];
                $bike_id = $rental['id_bike'];
                
                error_log("Start station ID: " . $start_station_id);
                error_log("Bike ID: " . $bike_id);
                ...
                if ($start_station_id) {
                    ...
                    $result = $updateStation->execute(['id' => $start_station_id]);
                    error_log("Update station result: " . ($result ? "success" : "failed"));
                    
                    // Check if the update actually happened
                    $checkStation = $pdo->prepare("SELECT available_bikes FROM BikeStation WHERE id_station = :id");
                    $checkStation->execute(['id' => $start_station_id]);
                    $station = $checkStation->fetch();
                    error_log("Station available bikes after update: " . print_r($station, true));
                } else {
                    error_log("No start station ID found in rental");
                }
                
                // 🔧 Remettre le vélo en disponible
                $updateBike = $pdo->prepare("UPDATE Bike SET status = 'Inactive' WHERE id_bike = :bike_id");
                $result = $updateBike->execute(['bike_id' => $bike_id]);
                error_log("Update bike result: " . ($result ? "success" : "failed"));
            } else {
                error_log("Rental is already ended, no need to increment bikes");
            }
            
            // 🗑️ Supprimer la location
            $deleteRental = $pdo->prepare("DELETE FROM BikeRental WHERE id_rental = :id AND id_user = :user");
            $result = $deleteRental->execute(['id' => $rental_id, 'user' => $id_user]);
            error_log("Delete rental result: " . ($result ? "success" : "failed"));
        } else {
            error_log("No rental found with ID: " . $rental_id);
        }
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto p-4 p-lg-0">
                ...
                <div class="dropdown-menu fade-up m-0">
                        ...
                        <a href="Stations.php" class="dropdown-item active">Velos et Stations</a>
